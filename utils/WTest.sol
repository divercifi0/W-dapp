...
contract WarrantyManager {
    ...
    event WarrantyCreated(uint256 serialNumber, string coverageDetails, uint256 durationInMonths, address manufacturer);
    event WarrantyClaimed(uint256 serialNumber, uint256 claimTimeStamp, address customer, address manufacturer);
    ...
    function createWarranty(uint256 serialNumber, string memory coverageDetails, uint256 durationInMonths) public {
    ...
    function claimWarranty(uint256 serialNumber) public {
    ...
    function voidWarranty(uint256 serialNumber) public {
        require(warranties[serialNumber].status == WarrantyStatus.Active, "Warranty not active");
        warranties[serialNumber].lastUpdated = block.timestamp;
    }

    function getWarrantyStatus(uint256 serialNumber) public view returns (WarrantyStatus) {
        return warranties[serialNumber].status;
    }

    function getWarrantyBySerialNumber(uint256 serialNumber) public view returns (Warranty memory) {
